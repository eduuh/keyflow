 ...
 * NOTE: With the synchronous cleanup fix, cleanup happens immediately on key release.
 */
...
TEST(PhantomCapsFixTest, FastTypingAfterShiftCombo) {
  ...
  EXPECT_FALSE(trackerPtr->hasInjectedShift()) << "Injected shift cleared immediately";
...
TEST(PhantomCapsFixTest, MultipleShiftCombosInSequence) {
  ...
  EXPECT_FALSE(trackerPtr->hasInjectedShift()) << "Cleanup done immediately after backtick";
...
TEST(PhantomCapsFixTest, CleanupHappensOnKeyUp) {
  ...
  EXPECT_FALSE(trackerPtr->hasInjectedShift()) << "Cleanup done immediately on key release";
}
...
TEST(PhantomCapsFixTest, RapidRepeatedCombo) {
  ...
  for (int i = 0; i < 3; i++) {
    // Press
    simulateKeyEvent(pipeline, *trackerPtr, KeyEvent{SC_GRAVE, true});
    EXPECT_TRUE(trackerPtr->hasInjectedShift()) << "Shift injected on press " << i;

    // Release (cleanup happens immediately)
    simulateKeyEvent(pipeline, *trackerPtr, KeyEvent{SC_GRAVE, false});
    EXPECT_FALSE(trackerPtr->hasInjectedShift()) << "Cleanup done immediately after combo " << i;
  }
